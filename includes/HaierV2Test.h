...
class Haier : public Climate, public PollingComponent, public uart::UARTDevice {
	...
	private:
		...
		byte initialization_1[13] = { 0xFF, 0xFF, 0x0A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x61, 0x00, 0x07, 0x72 };
		byte initialization_2[13] = { 0xFF, 0xFF, 0x08, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x70, 0xB8, 0x86, 0x41 };
		...
		void CompareStatusByte() {
			...
			for (i = 0; i < sizeof(status); i++) {
				if (status[i] != previous_status[i]) {
					ESP_LOGD(TAG, "Status byte %d: 0x%X --> 0x%X ", i, previous_status[i],status[i]);
				}
				previous_status[i] = status[i];
			}
		}
	...
	public:
		...
		void setup() override {
			this->set_timeout(1000, [this] { this->sendFirstInitSequence(); });
		}

		void sendFirstInitSequence() {
			this->write_array(initialization_1, sizeof(initialization_1));
			auto raw = format_hex_pretty(initialization_1, sizeof(initialization_1));
			ESP_LOGD(TAG, "initialization_1: %s ", raw);
			this->set_timeout(1000, [this] { this->sendSecondInitSequence(); });
		}

		void sendSecondInitSequence() {
			this->write_array(initialization_2, sizeof(initialization_2));
			auto raw = format_hex_pretty(initialization_1, sizeof(initialization_1));
			ESP_LOGD(TAG, "initialization_2: %s ", raw);
		}

		void loop() override {
			byte data[47];
			if (this->available() > 0) {
				if (this->readByte() != 255) return;
				if (this->readByte() != 255) return;
		...
		void update() override {
			...
			auto raw = format_hex_pretty(poll, sizeof(poll));
			ESP_LOGD(TAG, "POLL: %s ", raw.c_str());
		}

		void parseStatus() {
			auto raw = format_hex_pretty(status, sizeof(status));
			ESP_LOGD(TAG, "Readed message ALBA: %s ", raw.c_str());

			byte check = getChecksum(status, sizeof(status));

			if (check != status[CRC_OFFSET(status)]) {
				ESP_LOGW(TAG, "Invalid checksum (%d vs %d)", check, status[CRC_OFFSET(status)]);
				return;
			}
			...
			if (current_temperature < MIN_VALID_INTERNAL_TEMP || current_temperature > MAX_VALID_INTERNAL_TEMP
				...
				ESP_LOGW(TAG, "Invalid temperatures");
				return;
			}
			...
			//ESP_LOGW(TAG, "HVAC Mode = 0x%X", GetHvacModeStatus());

			ESP_LOGW(TAG, "Power Status = 0x%X", GetPowerStatus());
			ESP_LOGW(TAG, "Purify status = 0x%X", GetPurifyStatus());
			ESP_LOGW(TAG, "Quiet mode Status = 0x%X", GetQuietModeStatus());
			ESP_LOGW(TAG, "Fast mode Status = 0x%X", GetFastModeStatus());

			//ESP_LOGW(TAG, "Fan speed Status = 0x%X", GetFanSpeedStatus());
			//ESP_LOGW(TAG, "Horizontal Swing Status = 0x%X", GetHorizontalSwingStatus());
			//ESP_LOGW(TAG, "Vertical Swing Status = 0x%X", GetVerticalSwingStatus());
			//ESP_LOGW(TAG, "Set Point Status = 0x%X", GetTemperatureSetpointStatus());
			CompareStatusByte();
		...
		void control(const ClimateCall &call) override {
			...
			ESP_LOGD(TAG, "Control call");

			if (first_status_received == false) {
				ESP_LOGD(TAG, "No action, first poll answer not received");
				return;
			}
			...
			if (call.get_mode().has_value()) {
				...
				ESP_LOGD(TAG, "*call.get_mode() = %d", new_mode);
			...
			if (call.get_target_temperature().has_value()) {
				...
				ESP_LOGD(TAG, "*call.get_target_temperature() = %f", temp);
		...
		void sendData(byte *message, byte size) {
			...
			auto raw = format_hex_pretty(message, size);
			ESP_LOGD(TAG, "Message sent: %s  - CRC: %X - CRC16: %X", raw.c_str(), crc, crc_16);
		}
		...
		byte getChecksum(const byte *message, size_t size) {
			...
			if (size < ( position)) {
				ESP_LOGE(TAG, "frame format error (size = %d vs length = %d)", size, message[2]);
				return 0;
			}
