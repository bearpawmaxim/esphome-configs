...
class Clock {
  ...
  private:
    ...
    static void draw_bezel_(Display *display, uint16_t center_x,
      ...
      for (int i = 0; i < 60; ++i) {
        display->line(tick_values_[i][0], tick_values_[i][1],
          tick_values_[i][2], tick_values_[i][3]);
      }
    }

    static void draw_minutes_hand_(Display *display, uint16_t x, uint16_t y,
    ...
    static void draw_hours_hand_(Display *display, uint16_t x, uint16_t y,
        uint16_t radius, ESPTime time, Color color) {
  ...
  public:
    ...
    static void draw_watch_face(Display *display, uint16_t center_x,  uint16_t center_y,
