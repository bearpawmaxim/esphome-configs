...
private:
	...
	//byte initialization_1[13] = {0xFF,0xFF,0x08,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x09,0x11};
	//byte initialization_2[13] = {0xFF,0xFF,0x08,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x09,0x11};
...
public:
	...
	Haier(UARTComponent* uart) : PollingComponent(5 * 1000), UARTDevice(uart) {
		lastCRC = 0;
		_serial = new UARTWrapper(uart);
	}

	void setup() override {
		_serial->begin(9600);
		delay(1000);
		_serial->write(initialization_1, sizeof(initialization_1));
		auto raw = getHex(initialization_1, sizeof(initialization_1));
		ESP_LOGD("Haier", "initialization_1: %s ", raw.c_str());
		delay(1000);
		_serial->write(initialization_2, sizeof(initialization_2));
	...
	void loop() override {
		...
		if (_serial->available() > 0) {
			ESP_LOGD("Haier", "Available: %d ", _serial->available());
